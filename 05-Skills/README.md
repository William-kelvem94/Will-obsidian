---
title: "Skills"
date: 2026-06-07
updated: 2026-06-07
type: moc
status: active
tags: [skills, habilidades, hub, organizacao]
summary: "Entrada física da nova área dedicada às habilidades técnicas do vault."
---

# Skills

Esta pasta é o destino físico futuro para habilidades técnicas e documentação reutilizável.

## Navegação atual

- [[../01-Hubs/Hub-Skills|Hub de Skills]]
- [[skills/README|Skills atual]]
- [[../04-Conhecimentos/Conhecimentos-Gerais/06-Habilidades/Mapa-de-Habilidades]]

## Regra atual

A pasta `skills/` ainda preserva o conteúdo original. A migração física deve ser feita depois de revisar links e dependências internas.
