---
title: "Skills - Index"
date: 2026-06-10
updated: 2026-06-13
type: moc
status: active
tags: [skills, index, taxonomy, roadmap]
summary: "Indice central da taxonomia de skills com foco em cobertura, padrao e expansao."
---

# Skills - Index

## Pontos de entrada

- [[README|README de Skills]]
- [[ROADMAP-EXPANSAO|Roadmap de Expansao de Skills]]
- [[../skills/01-agentic-intelligence/README|Agentic Intelligence]]
- [[../skills/02-software-engineering/README|Software Engineering]]
- [[../skills/03-infrastructure-mcp/README|Infrastructure MCP]]
- [[../skills/04-knowledge-systems/INDEX|Knowledge Systems]]
- [[../skills/ai/INDEX|AI]]
- [[../skills/devops/Observabilidade|DevOps e Observabilidade]]
- [[../skills/frontend/INDEX-Addon|Frontend]]

## Prioridades de cobertura

1. padrao canonico de skill;
2. trilhas rasas com maior utilidade imediata;
3. sincronizacao entre `05-Skills/`, `.agents/skills` e `.continue/skills`;
4. metricas de uso e qualidade;
5. expansao dos temas ausentes.
