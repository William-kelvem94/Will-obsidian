...
# Mapeamento de pastas específicas para a reestruturação refinada
REFINED_MAPPING = {
    "04-Conhecimentos/07-Humanidades/Programacao": "04-Conhecimentos/02-Engenharia-de-Software",
    "04-Conhecimentos/07-Humanidades/Vida-Pratica": "04-Conhecimentos/08-Vida-Pratica",
    "04-Conhecimentos/07-Humanidades/Saude": "04-Conhecimentos/08-Vida-Pratica/Saude-Rotina",
    "04-Conhecimentos/Conhecimentos-Gerais": "04-Conhecimentos",
    "04-Conhecimentos/Knowledge-Base": "04-Conhecimentos",
    "04-Conhecimentos/wiki": "04-Conhecimentos/00-Mapas-e-Ontologia"
}
...
def build_refined_mappings(root_dir):
    old_to_new = {}
    
    # Mapear arquivos contidos nas pastas de origem
    for src_folder, dst_folder in REFINED_MAPPING.items():
        ...
        if os.path.exists(src_path) and os.path.isdir(src_path):
            for dirpath, _, filenames in os.walk(src_path):
                for filename in filenames:
                    ...
                    # Calcular o novo caminho relativo substituindo o prefixo da pasta antiga pelo da nova
...
def resolve_and_update_link(target, source_new_rel, old_to_new, new_to_old):
    ...
    if is_relative:
        ...
        if path_part_norm in old_to_new:
            ...
            # Buscar correspondências de caminhos absolutos baseados nos prefixos mapeados
            for src_folder, dst_folder in REFINED_MAPPING.items():
                src_folder_slash = src_folder.replace('\\', '/')
                dst_folder_slash = dst_folder.replace('\\', '/')
...
def update_links_in_file(filepath, root_dir, old_to_new, new_to_old, dry_run=True):
    ...
    # 3. Atualizar caminhos textuais ou referenciados no conteúdo das notas
    for src_folder, dst_folder in REFINED_MAPPING.items():
        src_folder_slash = src_folder.replace('\\', '/')
        dst_folder_slash = dst_folder.replace('\\', '/')
...
def main():
    parser = argparse.ArgumentParser(description="Reorganiza o vault Will-obsidian de forma refinada.")
    ...
    print(f"=== Reorganização Refinada do Vault ===")
    print(f"Diretório raiz: {root_dir}")
    print(f"Modo: {'APLICAR' if args.apply else 'SIMULAÇÃO (dry-run)'}")
    
    # 1. Construir mapeamentos refinados
    old_to_new = build_refined_mappings(root_dir)
    new_to_old = {v: k for k, v in old_to_new.items()}
    
    # 2. Planejar movimentações físicas
    moves_planned = []
    remove_duplicates = []
    ...
    # 3. Executar/Simular movimentações físicas
    ...
    # 4. Escanear e atualizar links em arquivos markdown
    ...
    # 5. Limpeza de Pastas Vazias
    print(f"\n--- Limpeza de Pastas Vazias ---")
    # Limpar as pastas mapeadas se estiverem vazias
    for folder in sorted(REFINED_MAPPING.keys(), key=len, reverse=True):
        ...
        if os.path.exists(path) and os.path.isdir(path):
            for dirpath, dirnames, filenames in os.walk(path, topdown=False):
                ...
                if not dirnames and not valid_files:
                    ...
                    if not dry_run:
                        try:
                            # Se restarem arquivos como .gitkeep e a pasta deve sumir, removemos
                            for f in filenames:
