...
```mermaid
flowchart TD
    A[Entrada: Tarefas e Demandas] --> B{Capturar tudo}
    B --> C[Caixa de Entrada]
    C --> D{Clarificar}
    D --> E[É acionável?]
    E -->|Sim| F[Qual a próxima ação?]
    E -->|Não| G[Lixo / Referência / Talvez]
    F --> H{Leva menos de 2 min?}
    H -->|Sim| I[Faça agora]
    H -->|Não| J{Delegar?}
    J -->|Sim| K[Delegue e aguarde]
    J -->|Não| L{Contexto / Energia / Tempo}
    L --> M[Organizar no sistema]
    M --> N[Refletir semanalmente]
    N --> O[Engajar e executar]
    O --> P[Concluído]
    I --> P
    K --> P
    G --> P
...
```mermaid
flowchart LR
    subgraph Ciclo_Ultradiano
        A1[90 min de Foco] --> A2[Pausa de 5-15 min]
        A2 --> A3[90 min de Foco]
        A3 --> A4[Pausa de 5-15 min]
        A4 --> A5[90 min de Foco]
        A5 --> A6[Pausa Longa 30 min]
    end
```
