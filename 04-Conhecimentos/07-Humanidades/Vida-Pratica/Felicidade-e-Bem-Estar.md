...
mindmap
  root((Felicidade))
    ...
    Economia
      ...
      Consumo conspícuo
      Bens relacionais
      Prosperidade além do PIB
      Economia do bem-estar
```

```mermaid
flowchart TD
    A[Felicidade] --> B[Hedonia]
    A --> C[Eudaimonia]
    B --> D[Prazer]
    B --> E[Satisfação]
    B --> F[Bem-Estar Subjetivo]
    C --> G[Significado]
    C --> H[Virtude]
    C --> I[Florescimento]
    D --> J[Dopamina]
    E --> K[Serotonina]
    G --> L[Córtex Pré-Frontal]
    H --> M[Caráter]
    I --> N[Propósito]
    N --> O[Resiliência]
    O --> P[Saúde]
...
flowchart LR
    ...
    subgraph Neurociência
        D["Dopamina<br/>+<br/>Ondas Theta"]
        E["Ativação<br/>Córtex Pré-Frontal"]
        F["Amígdala<br/>Regulada"]
    end
    subgraph Filosofia
        G[Virtude]
        H[Ataraxia]
...
```mermaid
timeline
    title Evolução Histórica do Conceito de Felicidade
    Antiguidade : Aristóteles (Eudaimonia) : Epicuro (Ataraxia) : Estoicismo (Virtude) : Budismo (Nirvana)
    : Sócrates (Conhece-te a ti mesmo)
    Idade Média : Agostinho (Felicidade em Deus) : Tomás de Aquino (Visão Beatífica)
    : Fé como caminho para felicidade
    Iluminismo : Bentham (Maior felicidade) : Mill (Prazeres superiores)
    : Kant (Felicidade vs Dever)
    : Rousseau (Felicidade natural)
    Séc. XIX : Utilitarismo dominante : Crítica Marxista (alienação)
    : Nietzsche (Vontade de poder)
    Séc. XX : Freud (frustração inevitável) : Frankl (Vontade de significado)
    : Rogers (Autorrealização) : Maslow (Hierarquia)
    : Existencialismo (Liberdade angustiada)
    Séc. XXI : Seligman (PERMA) : Csikszentmihalyi (Flow)
    : Fredrickson (Broaden-and-Build)
    : Ciência da Felicidade (evidência empírica)
    : Críticas: Ehrenreich, Cabanas, Davies
```
