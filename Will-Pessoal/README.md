---
title: "Will-Pessoal — Vida e Conhecimento"
description: "Pasta principal para o segundo cérebro pessoal de William: perfil, vida, aprendizado, rede e objetivos." 
tags:
  - pessoal
  - segundo-cerebro
  - will-pessoal
  - pk
  - hub
  - perfil
updated: 2026-04-27
date: 2026-04-27
---

# Will-Pessoal — Vida e Conhecimento

Esta pasta é a extensão pessoal do vault: o lugar onde Will documenta seu perfil, hábitos, finanças, saúde, aprendizado e conexões.

## Estrutura

- [[Will-Pessoal/01-Identidade/Perfil/README|Perfil]] — identidade, carreira, habilidades e visão pessoal.
- [[Will-Pessoal/03-Vida-Estilo/Vida/Habitos|Hábitos]] — rotina, trackers e hábitos de alto impacto.
- [[Will-Pessoal/03-Vida-Estilo/Vida/Financas|Finanças]] — orçamento, metas financeiras e controle de fluxo.
- [[Will-Pessoal/03-Vida-Estilo/Vida/Saude|Saúde]] — bem-estar, energia e cuidados pessoais.
- [[Will-Pessoal/03-Vida-Estilo/Conhecimento/Leituras|Conhecimento]] — leituras, cursos e aprendizagem contínua.
- [[Will-Pessoal/04-Social/Rede/Pessoas|Rede]] — contatos, networking e parcerias.
- [[Will-Pessoal/02-Visao/Objetivos/README|Objetivos]] — metas, OKRs e visão de curto/médio prazo.
- [[Will-Pessoal/01-Identidade/Perfil/INDEX|Perfil — Índice completo]]
- [[Will-Pessoal/Ideia_Pessoal|Ideia Pessoal]] — ideias e insights em andamento.

## Como usar

1. Abra `Will-Pessoal/README` para ver o panorama pessoal do vault.
2. Atualize o perfil sempre que novas metas, habilidades ou decisões pessoais surgirem.
3. Use a seção `Vida` para documentar hábitos, finanças e saúde como um sistema vivo.
4. Conecte tudo com `Projetos`, `JARVIS` e `Objetivos`.

## Resultado esperado

- Mais clareza sobre quem você é e para onde está indo.
- Menos mistura entre projetos de código e decisões de vida.
- Melhor uso do gráfico e do RAG ao incluir contexto pessoal relevante.
