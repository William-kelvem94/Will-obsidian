  ...
  root(("Religiões Comparadas"))
    ...
    (Filosofia)
      ...
      [Ética]
        ...
        Regra de Ouro (todas religiões)
      [Epistemologia]
        Fé vs Razão
        Revelação vs Ciência
        Teologia natural
    (Antropologia)
      [Estrutura]
        Mito e Ritual (Lévi-Strauss)
        Ritos de passagem (Van Gennep)
      [Símbolo]
        Símbolos religiosos (Geertz)
        O sagrado e o profano (Eliade)
      [Cosmovisão]
        Orixás, ancestrais, espíritos
        Xamanismo e cura
    (História)
      [Tradições]
        Judaísmo → Cristianismo → Islamismo
        Hinduísmo → Budismo → Jainismo
        Religiões africanas / diaspóricas
      [Eventos]
        Êxodo, Exílio, Holocausto
        Concílios, Cisma, Reforma
        Hégira, Cruzadas, Colonialismo
    (Psicologia)
      [Freud]
        Deus como projeção paterna
        Religião como neurose obsessiva
      [Jung]
        Arquétipos do inconsciente coletivo
        Self como imagem de Deus
        Símbolos universais (mandala, árvore, água)
      [James]
        Experiência religiosa (Variedades)
        Conversão, misticismo, santidade
      [Frankl]
        Sentido da vida (logoterapia)
        Deus como sentido último
    (Sociologia)
      [Durkheim]
        Sagrado vs Profano
        Religião como coesão social
      [Weber]
    ...
    (Política)
      ...
      [Estado e Religião]
        ...
        Fundamentalismo
      [Diálogo]
        Paz mundial e religiões
        Hans Küng: "Não haverá paz no mundo sem paz entre as religiões"
    (Ética)
      [Bioética]
        Aborto, eutanásia, clonagem
      [Guerra e Paz]
        Guerra justa
        Pacifismo
      [Meio Ambiente]
        Ecologia profunda
        Administração divina (stewardship)
        Ahimsa (não-violência)
```

```mermaid
graph TD
    subgraph "Relações históricas"
        J[Judaísmo] --> C[Cristianismo]
        J --> I[Islamismo]
        C --> P[Protestantismo]
        I --> S[Sufismo]
        H[Hinduísmo] --> B[Budismo]
        H --> Jn[Jainismo]
        C --> AF[Rel. Afro-Brasileiras]
        AF --> U[Umbanda]
        AF --> CD[Candomblé]
    end

    subgraph "Conceitos compartilhados"
        RG[Regra de Ouro]
        RG --> C
        RG --> I
        RG --> H
    ...
    subgraph "Diferenças estruturais"
        D1[Monoteísmo] --> C
        D1 --> I
        D1 --> J
        D2[Reencarnação] --> H
