...
```mermaid
graph TD
    T[Taoísmo] --> F[Taoísmo Filosófico<br/>Daojia]
    T --> R[Taoísmo Religioso<br/>Daojiao]
    
    F --> L[Laozi / Tao Te Ching]
    F --> Z[Zhuangzi]
    F --> W[Wu Wei / Ziran / Pu]
    
    R --> A[Alquimia Interna<br/>Neidan]
    R --> P[Panteão de Deuses<br/>e Imortais]
    R --> RI[Rituais e Cultos]
    R --> FC[Feng Shui]
    R --> TC[Tai Chi / Qi Gong]
    
    T --> Y[Yin-Yang]
    Y --> M[Medicina Chinesa]
    Y --> E[Astrologia / I Ching]
    Y --> A2[Artes Marciais]
    
    T --> N[Harmonia com a Natureza]
    N --> EC[Ecologia Profunda]
    N --> S[Sazonalidade]
    
    F --> CONF{{Confucionismo}}
    F --> BUD{{Budismo Chan/Zen}}
    F --> EST{{Estética Chinesa<br/>Pintura, Poesia, Caligrafia}}
    
    CONF --> RIT[Rituais e Hierarquia]
    CONF --> MOR[Educação Moral]
    BUD --> MED[Meditação<br/>Vazio / Talidade]
    EST --> SIM[Simplicidade / Assimetria<br/>Vazio Fértil]
    
    M --> ACU[Acupuntura]
    M --> ERV[Fitoterapia]
    M --> QI[Qi - Energia Vital]
```
